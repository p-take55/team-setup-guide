...
6. **人間にしかできない登録は、まとめて一度だけ具体的に渡す。** 会話に出た人・取引先・契約は agent には登録できないので、構成の整理を締めるときに「誰を・何を・どこから登録するか」を一覧で見せ、今やるか後にするかを選んでもらう。「後で案内します」と言ったまま次の話題へ進まない。渡した一覧は `PROJECT.md` にも残す。
